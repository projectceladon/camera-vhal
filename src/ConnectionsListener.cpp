...
ConnectionsListener::ConnectionsListener(std::string suffix)
    ...
    mSocketListenerThread = std::unique_ptr<std::thread>(new std::thread(&ConnectionsListener::socketListenerThreadProc,this));
}
...
bool ConnectionsListener::socketListenerThreadProc() {
    ...
    while (mRunning) {
        ...
        if (ret <= 0)
        {
            ALOGV("%s: poll() failed or poll timeout", __FUNCTION__);
            continue;
        }
